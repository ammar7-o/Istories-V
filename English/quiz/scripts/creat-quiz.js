...
function loadUserQuizzes() {
    ...
    userQuizzes.forEach((quiz, index) => {
        ...
        quizItem.innerHTML = `
            ...
            <div class="story-item-actions">
                ...
                <button class="story-action-btn start-quiz-btn" title="Start Quiz" data-index="${index}" data-quiz-id="${quiz.id}">
    ...
    document.querySelectorAll('.preview-quiz-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            previewQuiz(userQuizzes[index]); // CORRECT - passing quiz object
        });
    });
...
// Function to start quiz from Add Quizzes page
function startQuizFromAddQuizzes(quizId) {
    // Make sure we're showing quizzes on home page
    const selectElement = document.getElementById('Select');
    if (selectElement) {
        selectElement.value = 'quizzes';
        // Trigger change event to show quizzes
        selectElement.dispatchEvent(new Event('change'));
    }

    // Wait for quizzes to render, then start the quiz
    setTimeout(() => {
        startQuiz(quizId);
    }, 200);
}

// Updated startQuiz function to handle user quizzes
function startQuiz(quizId) {
    console.log('Starting quiz with ID:', quizId);
    console.log('All quizzes:', quizzes.length);
    console.log('User quizzes:', userQuizzes.length);

    // First check in main quizzes array
    currentQuiz = quizzes.find(q => q.id === quizId);

    // If not found, check in user quizzes
    if (!currentQuiz) {
        console.log('Quiz not found in main array, checking user quizzes...');
        currentQuiz = userQuizzes.find(q => q.id === quizId);

        // If found in user quizzes, add it to main array
        if (currentQuiz) {
            console.log('Quiz found in user quizzes, adding to main array');
            quizzes.push(currentQuiz);
        }
    }

    if (!currentQuiz) {
        console.log('Quiz not found anywhere!');
        console.log('Available quiz IDs in quizzes:', quizzes.map(q => q.id));
        console.log('Available quiz IDs in userQuizzes:', userQuizzes.map(q => q.id));
...
// Update quiz in main quizzes array
function updateQuizInMainArray(userQuiz) {
    // Find the index in main quizzes array
    const quizIndex = quizzes.findIndex(q => q.id === userQuiz.id);

    if (quizIndex !== -1) {
        // Update existing quiz
        quizzes[quizIndex] = { ...userQuiz };
    } else {
        // Add new quiz to main array
        quizzes.push({ ...userQuiz });
    }
    console.log('Updated quiz in main array:', userQuiz.id);
}
...
// In previewQuizFromEdit() function:
function previewQuizFromEdit() {
    const quiz = userQuizzes[currentEditQuizIndex];
    if (quiz) {
        closeEditQuizModal();
        previewQuiz(quiz); // Pass the quiz object, not quiz.id
    }
}
// Add this function to handle preview by ID (for other parts of your app):
function previewQuizById(quizId) {
    // Check in main quizzes array
    ...
    if (quiz) {
        ...
        showNotification('Quiz not found for preview.', 'error');
    }
}

// Then update your event listeners to use this:
document.querySelectorAll('.preview-quiz-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
        const index = parseInt(e.currentTarget.dataset.index);
        previewQuizById(userQuizzes[index].id); // Use the new function
    });
});
...
function testQuizFromEdit() {
    ...
    if (quiz) {
        ...
        // Wait for page to switch, then start the quiz
        setTimeout(() => {
            startQuizFromAddQuizzes(quiz.id);
        }, 100);
    }
}
...
// Load all user quizzes into main quizzes array on app initialization
function loadUserQuizzesIntoMainArray() {
    userQuizzes.forEach(quiz => {
        // Check if quiz already exists in main array
    ...
    console.log('Loaded user quizzes into main array. Total quizzes:', quizzes.length);
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Load user quizzes into main quizzes array
    loadUserQuizzesIntoMainArray();
