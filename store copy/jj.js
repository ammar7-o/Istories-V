...
// ========= INTEGRATED: Open Story in New Page ==========
function openStoryInNewPage(storyId) {
    // First, try to find the story in user stories
    let story = userStories.find(s => s.id == storyId);
    
    // If not found in user stories, try main stories
    if (!story) {
        story = stories.find(s => s.id == storyId);
    }

    if (story) {
        // Get translations for this story if it's a user story
        let translations = {};
        ...
        // Determine the correct reader path
        let storyPage = '';
        if (window.location.pathname.includes('/English/')) {
            storyPage = '../English/reader/index.html?id=' + storyId + (story.isUserStory ? '&userStory=true' : '');
        } else if (window.location.pathname.includes('/add-stories.html')) {
            storyPage = '../English/reader/index.html?id=' + storyId + '&userStory=true';
        } else {
            storyPage = '../English/reader/index.html?id=' + storyId + (story.isUserStory ? '&userStory=true' : '');
        }
...
function downloadStory(storyId) {
    // Find the story
    const story = stories.find(s => s.id == storyId) || userStories.find(s => s.id == storyId);
    ...
    try {
        ...
        const storyForDownload = {
            ...
            ...(story.isUserStory && userDictionaries[story.id] && { translations: userDictionaries[story.id] }),
